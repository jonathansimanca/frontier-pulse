...
6. **Factual Claim Check (`src/claim_verifier.py`):** Prompts require source-faithful attribution, and one grounded Gemini call fact-checks the selected stories after selection, correcting only their text (never ids, sources, or scores). Corrections require search grounding and a contradicting source URL per removed claim; claims that merely could not be found are kept, and the original text stays if the check fails. Set `CLAIM_CHECK_ENABLED=false` to disable.
